 ...
 * Floating Popup 定位模型
 * ============================================================================
 * 抽象自浏览器插件里「选区气泡」的定位逻辑，通用化为：
 *
 *   在一个矩形范围 container（通常是浏览器 viewport）内，
 *   让 popup 围绕 anchor（选区包围盒，单行或多行选区的 bounding rect）定位。
 *
 * 规则：
 *   1. 按 sides 顺序（默认 右 → 下 → 左 → 上）逐个尝试，哪边能完整放下就放哪边。
 *      主轴贴着 anchor 留出 gap 呼吸空间；交叉轴对 anchor 居中，
 *      越界时沿交叉轴滑动（clamp 进安全区），不算失败。
 *   2. popup 永远不越出 container 内缩 safePadding 后的「安全区」，
 *      不会被 container 裁切（除非 popup 本身比安全区还大，此时贴左上角）。
 *   3. 四边都放不下时进入 fallback：把各方向候选位置 clamp 进安全区，
 *      选出遮挡 anchor 面积最小的那个 —— 尽量不遮住用户可见的选区。
 *
 * 纯函数、无 DOM 依赖：传 DOMRect 或普通对象都可以。
 * ============================================================================
 */
...
function intersectArea(left: number, top: number, size: Size, rect: Rect): number {
  const w = Math.min(left + size.width, rect.left + rect.width) - Math.max(left, rect.left);
  const h = Math.min(top + size.height, rect.top + rect.height) - Math.max(top, rect.top);
  return Math.max(0, w) * Math.max(0, h);
}

/**
 * 带过程信息的版本：除最终结果外，返回四个方向的候选位置与可行性，
 * 方便调试 / 可视化。日常使用直接调 computePopupPosition 即可。
 */
export function explainPopupPosition(
  ...
  const ideal: Record<Side, { left: number; top: number }> = {
    ...
    bottom: { left: clamp(centeredLeft, minLeft, maxLeft), top: anchorBottom + gap },
  ...
  const candidates: SideCandidate[] = sides.map((side) => {
    const c = ideal[side];
    const fits =
      containable && c.left >= minLeft && c.left <= maxLeft && c.top >= minTop && c.top <= maxTop;
    // 不可行的方向按 clamp 进安全区后的位置参与兜底比较
    const left = clamp(c.left, minLeft, maxLeft);
    const top = clamp(c.top, minTop, maxTop);
    return { side, left, top, fits, overlapArea: intersectArea(left, top, popup, anchor) };
  });
  ...
  if (clean) {
    return {
      result: { left: clean.left, top: clean.top, side: clean.side, fallback: false, overlapArea: 0 },
      candidates,
    };
  }
 ...
 *
 * @param anchor    选区矩形（SelectedRect），与 container 同坐标系
 * @param popup     popup 的尺寸（FloatingPopup 宽高）
 * @param container 限定范围（ContainerRect），viewport 场景传 {left:0, top:0, width:innerWidth, height:innerHeight}
 */
