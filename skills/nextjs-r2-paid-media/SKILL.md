...
- 公开域名上的模糊缩略图等展示层图片，可配合 [[nextjs-cloudflare-image-loader]] skill 做响应式优化（/cdn-cgi/image URL 转换）。但**付费原图严禁**用 URL 转换做模糊付费墙——源图必须公开可访问，删掉 URL 里的转换段就是原图；模糊图必须离线生成（本 skill 的 upload-media.ts 方案）。
