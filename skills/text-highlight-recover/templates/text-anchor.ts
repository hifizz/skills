...
export function buildQuoteSelector(snapshot: string, start: number, end: number): TextQuoteSelector {
  return {
 ...
 */
export function fuzzySubstring(text: string, pattern: string, hint?: number): FuzzyMatch | null {
  ...
  for (let j = 1; j <= n; j++) {
    ...
    if (hint != null && bestJ !== -1) {
      if (Math.abs(prevFrom[j] - hint) < Math.abs(prevFrom[bestJ] - hint)) bestJ = j
    }
  }
 ...
 */
export function paintRange(range: Range, id: string, color = 'rgba(255, 214, 0, 0.4)'): boolean {
